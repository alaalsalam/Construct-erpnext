...
أصبح بالإمكان عرض أساس تشغيلي للعمولات بدون ترحيل محاسبي:

- افتح Broker لعرض الوسيط `وسيط عقاري رئيسي`.
- افتح Commission Rule لعرض قاعدة `عمولة مبيعات عقارية 2.5%`.
- افتح Commission Entry `COM-2026-00001` المرتبط بعقد البيع `SC-PROJ-0002-001`.
- افتح Commission Register وBroker Performance Report وCommission Payable Summary لشرح العمولة المستحقة تشغيلياً.

ملاحظة العرض: العمولة هنا متابعة إدارية وتشغيلية فقط. لا يتم إنشاء سند دفع أو قيد يومية أو ترحيل GL للوسيط إلا بعد اعتماد تصميم دفع العمولات من الفريق المالي.

## تحديث CRM العقاري

أصبح بالإمكان عرض مسار CRM تشغيلي قبل الحجز:

- افتح Customer Requirement Register مع فلتر `REP-2026-00002`.
- افتح متطلبات العملاء `REQ-2026-00001` إلى `REQ-2026-00005` لشرح الشراء، الإيجار، الاستثمار، والـ Backlog.
- افتح Viewing Schedule Report لعرض مواعيد المعاينة `VA-2026-00001` إلى `VA-2026-00003`.
- افتح Follow Up Report لعرض المتابعات `REFU-2026-00001` إلى `REFU-2026-00004`.
- افتح Sales Pipeline by Requirement وLead Source Summary لشرح قيمة المسار ومصادر العملاء.

ملاحظة العرض: CRM لا يحجز وحدة ولا ينشئ عقداً أو فاتورة تلقائياً. التحويل يتم لاحقاً عبر الحجز أو العقد حسب الصلاحيات والإجراءات.

## تحديث المطابقة الذكية

أصبح بالإمكان عرض مطابقة متطلبات العملاء مع الوحدات المتاحة:

- افتح Match Result Register لعرض نتائج المطابقة `MATCH-2026-00001` إلى `MATCH-2026-00005`.
- افتح Recommended Units Report لشرح الوحدات المقترحة ودرجة المطابقة ومعايير المطابقة وغير المطابقة.
- افتح Matching Performance Summary لشرح أن بعض الطلبات لديها وحدات مناسبة وبعضها يحتاج Backlog.

ملاحظة العرض: المطابقة توصية فقط ولا تنشئ حجزاً. النظام يستبعد الوحدات غير المتاحة من التوصيات.

## تحديث Backlog Matching

أصبح بالإمكان عرض الطلبات التي لا تجد وحدة مناسبة حالياً:

- افتح Backlog Request Register لعرض `BLR-2026-00001` إلى `BLR-2026-00003`.
- افتح Market Demand Gap Report لشرح فجوة الميزانية أو المساحة أو النوع.
- افتح Backlog Matching Attempts Report لعرض محاولات إعادة المطابقة.

ملاحظة العرض: Backlog لا يرسل إشعاراً ولا يحجز وحدة تلقائياً. هو سجل طلب سوق يحتاج متابعة دورية من فريق المبيعات.

## تحديث الصيانة العقارية

أصبح بالإمكان عرض طلبات صيانة مرتبطة بالوحدات:

- افتح Maintenance Request Register مع فلتر `REP-2026-00002`.
- افتح الطلبات `PMR-2026-00001` إلى `PMR-2026-00004`.
- افتح Open Maintenance Requests لعرض الطلبات غير المكتملة.
- افتح Maintenance Cost Summary وMaintenance by Unit Report لشرح التكلفة التقديرية والفعلية حسب نوع المشكلة والوحدة.

ملاحظة العرض: الصيانة هنا تشغيلية فقط، ولا تنشئ أوامر عمل أو فواتير أو قيوداً محاسبية تلقائياً.

## تحديث وثائق العقار والعقود

أصبح بالإمكان عرض سجل وثائق مرتبط بالوحدات والعقود:

- افتح Property Document Register مع فلتر `REP-2026-00002`.
- افتح الوثائق `PDOC-2026-00001` إلى `PDOC-2026-00003`.
- افتح Expiring Documents Report لعرض الوثيقة التي قاربت الانتهاء.
- افتح Documents by Unit Report لشرح توزيع الوثائق حسب الوحدة.
