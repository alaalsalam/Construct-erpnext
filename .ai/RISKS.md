...
- `BOQ-PROJ-0002-001` remains Draft because direct workflow jumps are not allowed; Work Items already exist and are valid for reporting. Do not describe the BOQ status as a production approval unless it is formally approved through the workflow later.
...
- Project Purchase Control Summary was not created because existing procurement reports now show planned/requested/ordered/received/invoiced/consumed/remaining for PROJ-0002. If the client asks for a single-page procurement report, create it as a later report-only enhancement.
